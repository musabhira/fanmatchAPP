...
class FanMatchSupabaseUser extends BaseAuthUser {
  FanMatchSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> fanMatchSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FanMatchSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
